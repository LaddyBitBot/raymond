...
5. **One operator section.** Position alerts and action items merge into a single **"Operator decisions & watch levels"** block: one row per item — asset/topic, trigger level, action if hit, pointer to the analysis. No prose restatement of the reasoning.
