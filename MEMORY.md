...
- **When Mike says "open" or "close" a position, he is the trader — I only update the log.** I do not execute trades or place orders. The entry/exit price he states is logged as the operator-executed fill, and I commit/push the log update unless he tells me not to.
- **When replacing a file from an inbound staged document, verify the workspace file content before `git add`.** Don't assume `git add -A` caught the change — explicitly `read` the first few lines or run `git diff --stat` after staging. The 2026-07-20 rating-system-spec replacement was missed on the first pass because the staged file wasn't copied to the workspace root.
- **After every commit/push, run post-push verification:** `git status` (working tree clean), `git log --oneline -3`, and a spot-check of the file contents on disk. For GitHub, confirm the local source of truth is correct first; GitHub caching can lag, but a clean local push is the actionable evidence.
- **When a cron job halts per its own validation rules, do not "fix" the problem by restoring files from git or taking other unauthorized actions.** The halt is the system working as designed. Surface the halt to Mike and wait for his decision. Do not improvise a fix outside the job's defined authority.